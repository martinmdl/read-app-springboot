...
class Recomendacion(
    ...
    fun agregarObserver(observer: RecomendacionObserver) {
        observadores.add(observer)
    }

    fun sacarObserver(observer: RecomendacionObserver) {
    ...
    fun editarPrivacidad(usuarioQueEdita: Usuario) {
        if (esCreador(usuarioQueEdita)) {
            ...
            throw Businessexception("No es el creador")
        }
    ...
    fun editarDescripcion(usuarioQueEdita: Usuario, descripcionNueva: String) {
        if (esCreador(usuarioQueEdita) || esAmigo(usuarioQueEdita)) {
            ...
            throw Businessexception("No es el creador o no es amigo")
        }
    }
    ...
    fun agregarALibrosDeRecomendacion(usuarioQueEdita: Usuario, libro: Libro) {
        if (validarEdicion(usuarioQueEdita, libro)) {
            ...
            throw Businessexception("No se puede agregar un libro a la recomendacion")
        }
    }
    ...
    fun crearValoracion(valor: Int, comentario: String, usuario: Usuario) {
        if (puedeValorar(usuario)) {
            ...
            throw Businessexception("No se puede crear valoración")
        }
    }

    //falta testear
    fun puedeValorar(usuario: Usuario): Boolean =
        (amigoleidosTodos(usuario) || collecionAutorFavorito(usuario)) && usuario != creador

    private fun collecionAutorFavorito(usuario: Usuario): Boolean =
        librosRecomendados.all { it.autor == usuario.autorFavorito }

    /*GETTERS*/
    fun puedeLeerLaRecomendacion(usuarioQueLee: Usuario): Boolean = !esPrivado || esAmigoOCreador(usuarioQueLee)
    ...
    /*AUX*/
    fun esCreador(usuario: Usuario): Boolean = creador == usuario

    private fun esAmigo(usuario: Usuario): Boolean {
        return creador.amigos.contains(usuario)
    }

    private fun esAmigoOCreador(usuarioQueLee: Usuario): Boolean {
