...
class LectorPromedio(usuario: Usuario) : TipoLector(nameClass = "Lector Promedio", usuario = usuario)

class LectorNormal(usuario: Usuario) : TipoLector(nameClass = "Lector Normal", usuario) {
    override fun velocidadDeLectura(libro: Libro): Double =
        usuario.palabrasPorMinuto.toDouble()
}

class LectorAnsioso(usuario: Usuario) : TipoLector(nameClass = "Lector Ansioso", usuario) {
...
class LectorFanatico(usuario: Usuario) : TipoLector(nameClass = "Lector Fanatico", usuario) {
    ...
    private fun tiempoLecturaImportante(libro: Libro): Double =
        super.tiempoDeLectura(libro) + tiempoAdicionalImportante(libro)

    private fun tiempoAdicionalLargo(libro: Libro): Double =
        (libro.getMinimoPaginas() * COEFICIENTE_LIBRO_IMPORTANTE + (libro.paginas - libro.getMinimoPaginas())).toDouble()

    private fun tiempoAdicionalCorto(libro: Libro): Double = (libro.paginas * COEFICIENTE_LIBRO_IMPORTANTE).toDouble()
}

class LectorRecurrente(usuario: Usuario) : TipoLector(nameClass = "Lector Recurrente", usuario) {
    ...
    override fun velocidadDeLectura(libro: Libro): Double =
        super.velocidadDeLectura(libro) * (ENTERO - porcentajeDeDisminucion(libro))

    private fun porcentajeDeDisminucion(libro: Libro): Double =
        (checkLecturas(libro) * DISMINUCION_VELOCIDAD_LECTURA_POR_REPETICION)

    private fun checkLecturas(libro: Libro): Int =
        if (usuario.cantidadLecturas(libro) > LECTURAS_MAX) LECTURAS_MAX else usuario.cantidadLecturas(libro)
}
